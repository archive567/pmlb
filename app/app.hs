...
import Control.Lens hiding ((.>), (<|), Unwrapped, Wrapped, runFold, (|>), para)
...
-- stack build --exec "$(stack path --local-install-root)/bin/pmlb --datatype Classification --dataset saheart" --file-watch

-- $setup
-- >>> import PMLB
-- >>> import PMLB.Csv

-- | full csv parse of the defaultConfig config
-- >>> (Right (heads, rs, ())) <- runBS defaultCsvConfig $ parseCsvFull HasHeader '\t' 500 doubles
-- >>> heads
...
-- | no error parse
-- >>> (heads, rows) <- runBS defaultCsvConfig (parseCsv HasHeader 2 '\t' doubles)
-- >>> take 2 heads
-- ["X1","X2"]
--
-- >>> take 2 <$> rows
...
-- >>> (heads, rs) <- runBS defaultCsvConfig (parseCols HasHeader 5 '\t' (const double) (101,[2,3,5,100]))
-- >>> heads
-- ["X3","X4","X6","target"]
--
-- >>> rs
...
main = do
  ...
  let ds = fromMaybe (arb ^. #csvConfig . #dataSet) (dataset o)
  let dt = fromMaybe (arb ^. #setType) (settype o)
  let cfg = defaultPMLBConfig & #csvConfig . #dataSet .~ ds & #setType .~ dt
  cacheUrl cfg
  let c = cfg ^. #csvConfig
  recs <- runBS c (bsCheck c 10000)
  ranges <- runFold c 10000 doubles rangeFold
  freqs <- discreteFreqCounts c 5
  putStrLn (ds <> " 👍" :: Text)
  (hs, dss) <- runBS c (parseCsv HasHeader 5 '\t' doubles)
  void
    $ runOutput
    ...
    $ output "results"
    $ Native
      [ Para [Str "dataset: ", link (cfg ^. #csvConfig . #dataSet) (Text.pack $ url cfg)],
        plain $ "rectangle check (max 10000 lines):" <> show recs,
        plain "first few lines",
        table "first few lines" hs (replicate (length hs) AlignRight) (replicate (length hs) 0) (fmap show <$> dss),
        plain "ranges",
        table
          "ranges"
        ...
        code mempty mempty (pShow' freqs)
      ]
