...
-- >>> (Right (heads, rs, ())) <- runBS defaultConfig $ parseCsvFull HasHeader '\t' 500 doubles
-- >>> heads
...
-- | no error parse
-- >>> (heads, rows) <- runBS defaultConfig (parseCsv HasHeader 2 '\t' doubles)
-- >>> take 2 heads
-- ["X1","X2"]
--
-- >>> take 2 <$> rows
...
-- >>> (heads, rs) <- runBS defaultConfig (parseCols HasHeader 5 '\t' (const double) (101,[2,3,5,100]))
-- >>> heads
-- ["X3","X4","X6","target"]
--
-- >>> rs
...
main = do
  ...
  let cfg = defaultConfig & #dataSet .~ ds & #dataType .~ dt
  -- res <- (S.take 5 >>> S.toList_) & runLine cfg
  recs <- runBS cfg (bsCheck defaultConfig 10000)
  -- hs <- runBS arb (parseCsvHeader_ (arb ^. #csep))
  ranges <- runFold cfg 10000 doubles rangeFold
  freqs <- discreteFreqCounts cfg 5
  putStrLn (ds <> " 👍" :: Text)
  (hs, dss) <- runBS cfg (parseCsv HasHeader 5 '\t' doubles)
  void
    $ runOutput
    ...
    $ output "results"
    $ Native
      [ Para [Str "dataset: ", link (cfg ^. #dataSet) (Text.pack $ url cfg)],
